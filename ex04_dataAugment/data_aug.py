...
class RandomCrop(object):
    ...
    def __init__(self, crop=0.2, diff=False):
        self.crop = crop

        if type(self.crop) == tuple:
            assert len(self.crop) == 2, "Invalid range"
            assert self.crop[0] > 0 & self.crop[0] < 1
            assert self.crop[1] > 0 & self.crop[1] < 1


        else:
            assert self.crop > 0 and self.crop < 1
    ...
    def __call__(self, img, bboxes):
        ...
        # percentage of the dimension of the image to translate
        crop_factor = self.crope

        canvas = np.zeros(img_shape).astype(np.uint8)
        corner_x = int(translate_factor_x * img.shape[1])
        corner_y = int(translate_factor_y * img.shape[0])

        topleft_x = random.randint(corner_x)
        topleft_y = random.randint(corner_y)

        new_w = img.shape[1] - corner_x
        new_h = img.shape[0] - corner_y

        cropImg = img[topleft_x:topleft_x + new_w, topleft_y:topleft_y:new_h]

        img = cv2.resize(cropImg, img.shape, interpolation=cv2.INTER_AREA)
